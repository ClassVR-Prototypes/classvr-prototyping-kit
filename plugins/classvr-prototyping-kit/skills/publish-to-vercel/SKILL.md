...
Two ways, checked in this order. Everything below is written in terms of
*what* to do; the table says how on each route.

**1. The token route (kit 0.31+, the normal one — no connector).** The
person connected their Vercel account once with `/connect-vercel`; the token
lives in the built-in browser. Load the **Vercel bridge** exactly as
`/connect-vercel` "Loading the bridge" says (a browser-pane tab on
`https://api.vercel.com/v2/user`, then the bridge script), and check
`KV.status()`:

- connected and `await KV.check()` ok → use this route.
- not connected, or the token was refused → run `/connect-vercel` setup now
  (one short detour: make a token, paste it into the box), then continue.
  Do this even when the connector is also present — the token route is the
  one staff are set up for.
- no built-in browser in this session → go to 2.

**2. The Vercel connector (older set-ups).** Use it only when the token route
is unavailable (no built-in browser) and a deploy tool is present. Check for
the *capability*, never one tool name — Vercel renamed them on 21 Sep 2026.

**Neither** → stop and say: "Vercel steps need the Claude desktop app open on
your computer — open it and ask me again." (If `.classvr-kit.json` has no
`vercel.via` and there is no connector either, also offer
`/connect-vercel`.) Do not fall back to another route unasked.

| Need | Token route (bridge tab) | Connector |
|---|---|---|
| who am I / which team | `KV.status()` (`username`, `teamId`) | `get_auth_user`, `list_teams` |
| create the project, learn its real address | `await KV.ensureProject(name)` | `create_project`, then `get_project` domains |
| deploy files | paste the text `vercel_payload.py` wrote → `await KV.deploy({…})` | `create_deployment` (else `deploy_to_vercel`) |
| deployment state / alias | returned by `KV.deploy` | `get_deployment` |
| read a live page | a **second** browser tab (`preview_start`, `get_page_text`) | `web_fetch_vercel_url` |
| read the diary | `<url>/api/reports` in a second tab (public) | `get_runtime_logs`, `get_runtime_errors`, or `/api/reports` |
| keep play history | `await KV.createStore(name, projectId)` | `create_storage_stores_blob` |
| seed a file Vercel doesn't hold yet | send it inline (rebuild the payload without `reusable`) | `upload_file` |
| anything else | `await KV.api('/v…/…', {method, body})` — Vercel's REST API | the matching tool |

Keep the bridge tab on `api.vercel.com` for the whole publish; open published
pages in a second tab. **Wherever a step below says `web_fetch_vercel_url`**,
the token route reads the same public address in that second tab
(`get_page_text`), or with `fetch()` from it for JSON. Record which route was used as `vercel.via`
(`"token"` or `"connector"`) with the other `vercel.*` fields in step 6.
...
then **create the project and ask Vercel for its address** before building:
token route `await KV.ensureProject("<project name>")` → `url`; connector
`create_project` (framework none) then the project's domains. Pass that as
`--url <url>/`, so the page's QR code is right on the very first deploy.
Vercel **shortens long names** — `classvr-token-test-scene-lukemoseley-avantis`
is served at `classvr-token-test-scene-lukemosele.vercel.app` — so never
...
history viewer and, when the history has older versions, `/kit-relay.js` and
`/kit-qr.js` redirects to the library so pages published before kit 0.29
keep their diary and QR code), plus every older `v/<M>/index.html` it finds in the app's
`versions/` folder with a matching fingerprint (so `page/` is a complete
folder someone could drop into the Vercel dashboard; the connector route still
sends those by fingerprint) — plus `lib/*` (the library for this kit version:
`xr-kit-diary`, `xr-kit` .js/.css, `xr-kit-panel`, `kit-relay`, `kit-qr`,
`history-<kit>.html`, and the library project's own `vercel.json` and
`index.html`) and `manifest.json`. Since kit 0.29 the relay, QR script and
history page live in the library, not in each app. It also
...
Fetch `<libBase>/xr-kit-<kit>.js` (token route: `(await fetch('<libBase>/xr-kit-<kit>.js')).status`
on the bridge tab — the library allows any origin; connector: `web_fetch_vercel_url`).
...
- **Not found / error** → this template version has never been hosted. This
  ...
  `production`, files = **every file in `lib/`** exactly as the build wrote
  them (the manifest's `libDeploy` list: the seven library files plus
  `vercel.json` — `Cache-Control: public, max-age=31536000, immutable` and
  `Access-Control-Allow-Origin: *` on `/(.*)\.(js|css)` — and a one-line
  `index.html`). Token route: `vercel_payload.py "<scratch>/dist-vercel" --lib
  --out "<scratch>/lib-deploy.js"` and paste that file's text into
  `javascript_tool` on the bridge tab — it deploys and waits. Connector: wait
...
- The library is **shared by everyone** — its address is one worldwide name,
  so the first account to host a kit version serves it to every app built
  ...
  does this step; it stays here for a release that was missed.
...
Team: `vercel.teamId` from the manifest if set, else `KV.status().teamId`
(token route — the bridge adds it to every call for you), else `list_teams`
(one team → use it; several → ask once which).
...
`KV.status()` or `get_auth_user` (e.g. `classvr-bubble-pop-lukemoseley-avantis`). Lowercase,
...
**Token route** — let the script write the call, then paste it:

    python3 ${CLAUDE_PLUGIN_ROOT}/skills/publish-to-vercel/scripts/vercel_payload.py \
        "<scratch>/dist-vercel" --name "<project>" [--first] --out "<scratch>/deploy.js"

Read `<scratch>/deploy.js` and pass its **whole text, unchanged**, to
`javascript_tool` on the bridge tab. It inlines the new files, sends the rest
by fingerprint, checks each inline file's SHA-1 before anything leaves the
browser (a copying slip returns `stage: "check"` and nothing is sent — rebuild
the call and paste again), deploys and waits for READY. The result has `id`,
`projectId`, `alias` and `readyState`. `--first` on a first publish only.

**Connector route** — the same file list as one tool call:
    ...
    create_deployment  teamId = <team>   skipAutoDetectionConfirmation = "1"
      requestBody = {
        ...
        files: [ // new this publish — inline, from page/
                 ...
                 { file: "vercel.json",        data: <page/vercel.json>,        encoding: "utf-8" },
                 // unchanged since some earlier publish — by fingerprint, from deployFiles
                 { file: "api/log.js",       sha: <sha1>, size: <size> },
...
- **Inline what is new, fingerprint what is not.** `deployFiles` says which
  ...
  sent inline as `index.html` (its `/v/<N>/` address is a rewrite to it) and
  becomes the by-fingerprint `v/<N>/index.html` entry from the next publish on.
- **`missing_files` means Vercel does not hold that fingerprint yet** (a
  ...
  body base64) and deploy again by fingerprint (token route: rebuild the call
  with `--inline <file>` for each one). Never drop the file.

With the functions and a dependency install the deploy takes ~20 s; the
bridge waits for READY itself, on the connector poll `get_deployment`.

Then read `alias` (from `KV.deploy`, or `get_deployment`). **The public
address is the one `ensureProject` returned** — `<project>.vercel.app`, or
Vercel's shortened form of it for a long name; it must be in `alias`. The team-suffixed alias
...
(`<project>-<hash>-…`) either. If the expected address is missing, the project name
...
`vercel.lib=<kit>`, `vercel.via=token|connector`, and on a first publish `vercel.owner="<user's email>"`.
...
one call — do it right after the first deploy of an app, then never again:
    ...
    token route:  await KV.createStore("<project>-history", "<prj_…>")   → storeId, tokenSet
    connector:    create_storage_stores_blob  teamId = <team>
...
all by SHA, so it costs nothing; token route: `vercel_payload.py … --all-by-sha`) so the functions pick up the token. Record
...
Read `vercel.url` (token route: a **second** browser tab — `preview_start`,
then `get_page_text` / `javascript_tool` there, leaving the bridge tab on
`api.vercel.com`; connector: `web_fetch_vercel_url`): the text must contain
`window.BUILD = <N>;`, the `xr-kit-source` meta line and the six
`<libBase>/…-<kit>.js|css` references (four `xr-kit-…`, `kit-relay-…`,
`kit-qr-…`). Then `<vercel.url>/history.json`: its
...
and `<libBase>/kit-qr-<kit>.js` must answer 200. `<vercel.url>/history` must
redirect to `<libBase>/history-<kit>.html?app=<vercel.url>`. If
...
**Token route: go straight to the app's own history below.** The runtime-log
stream did not answer through the browser (24 Sep), and the history has the
same diary. A session appears there once the player leaves VR or closes the
page — if they're still playing, ask them to press the VR button to come out
and look again.

**Connector, within the hour — the runtime log, which has everything:**
...
**The app's own history** (token route always; connector for anything older than the hour), which every session wrote when
